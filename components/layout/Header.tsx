...
import { Bell, Search, Calendar, Shield, Users, ChevronDown, Check, UserCheck, ShieldAlert } from 'lucide-react';
...
export default function Header({
  ...
  const { currentUser, allUsers, switchUser, role, actingOnBehalfOf, setActingOnBehalfOf, isExecutive } = useAuth();
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <div className="hidden sm:block text-right">
              <div className="text-xs font-bold text-white leading-tight flex items-center gap-1.5">
                ...
                <span className={`text-[10px] px-1.5 py-0.2 rounded border font-semibold ${badgeStyle.bg} ${badgeStyle.text} ${badgeStyle.border}`}>
                  {currentUser.role}
                </span>
              </div>
          ...
          {isSwitcherOpen && (
            <div className="absolute left-0 mt-2 w-72 rounded-2xl bg-slate-900 border border-slate-700/90 shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
              ...
              {/* Task Delegation toggle for Executive */}
              {isExecutive && (
                <div className="mt-2 pt-2 border-t border-slate-800 px-3 py-2 bg-slate-800/40 rounded-xl">
                  <div className="flex items-center justify-between text-[11px] font-semibold text-amber-400">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300">متاح</span>
                  </div>
