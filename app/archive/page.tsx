  ...
  const systemAggregatedDocs = useMemo<ArchivedDocument[]>(() => {
    ...
    contracts.forEach((c) => {
      ...
      list.push({
        ...
        title: `عقد إيجار ${c.type === 'RESIDENTIAL' ? 'سكني' : 'تجاري'} (${cNum || 'جديد'})`,
        category: 'CONTRACT',
        ...
        notes: `عقد بقيمة سنوية ${(c.rent_amount || 0).toLocaleString('ar-SA')} ر.س - دورية السداد: ${c.payment_schedule || ''}`,
        tags: ['إيجار', c.type === 'RESIDENTIAL' ? 'سكني' : 'تجاري'],
    ...
    brokerageAgreements.forEach((b) => {
      ...
      list.push({
        ...
        status: b.ejar_status === 'Active' ? 'ACTIVE' : 'ARCHIVED',
        notes: `نسبة السعي: %${b.commission_rate || 0} • مدة الاتفاقية حتى ${b.expiry_date || ''}`,
        tags: ['وساطة', 'تسويق'],
